...
`tests/test_stylist_qlora_kaggle.py` adds coverage for the merged-bundle packaging path, confirms the requested three-model slate (`Qwen3-VL-8B-Instruct`, `Qwen3-VL-8B-Thinking`, `Qwen3.5-9B`), and verifies the Kaggle kernel script now imports `unsloth` before `transformers`, keeps W&B fallback via `WANDB_API_TOKEN`, and pins `transformers<5`/`huggingface_hub<1.0` while allowing the wheelhouse torch stack + post-install `bitsandbytes` repair logic for Kaggle compatibility.
...
`src/outfitmatch/kb/tagging.py` now implements Gemini/item semantic tagging for the graph path: validate enum-constrained `body_shapes_fit` / `season`, preserve free-form `colors`, cache responses with `diskcache`, and mutate `ItemRecord`s in place. The v2 stage-tagging gate rejects incomplete main-garment payloads so note-only/collapsed local-model outputs are retried instead of being written as complete tags. Operational CLI: `uv run python -m scripts.data.kb.tag_items` writes validated tags and colors back into `data/custom/catalog/catalog_metadata.parquet`.
