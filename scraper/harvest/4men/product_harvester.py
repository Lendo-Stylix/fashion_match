...
DISCOVERY_INPUT_FILE = os.path.join(BASE_DIR, "../../discovery/4men/output/scraped_products.json")
...
def parse_gtm_details(html_text):
    # Search for dataLayer.push with productDetail
...
def extract_specs(soup):
    ...
    desc_text = ""
    # Find accordion-content or details-box html-content containing "Chất liệu" or "Mã sản phẩm"
...
async def main():
    ...
    if not os.path.exists(DISCOVERY_INPUT_FILE):
        print(f"[!] Discovery file not found yet. Waiting for it...")
        # Check if the background task is running, wait up to 10 seconds
        for _ in range(10):
    ...
    # Load existing harvested products to support resume (anti-duplication)
    harvested_products = []
    seen_ids = set()
    seen_urls = set()
