...
Graph path (current architecture):
  ...
  6. traversal.py   — clique-safe outfit assembly from seed items
  7. assemble_record.py — derive OutfitRecord tags from assembled items
  8. graph_eval.py  — GraphReport (coverage / coherence / reuse)

Legacy materialized path (kept for comparison, see build_outfits.py):
  generation.py + scoring.py + tagging.py + qdrant_index.py (`outfits` collection).
